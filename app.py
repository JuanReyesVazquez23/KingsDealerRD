...
def init_db():
    ...
    with get_db() as conn:
        ...
        # Migración: columna imagenes_extra en anuncios_clientes
        conn.execute("""
            DO $$ BEGIN
              IF NOT EXISTS (
...
def api_vehiculos():
    ...
    # Solo dealer — particulares en /api/particulares
...
def api_particulares():
    """Solo clientes aprobados. Nunca mezclados con el dealer."""
    with get_db() as conn:
        rows = conn.execute(
            'SELECT id,marca,modelo,anio,tipo,precio,descripcion,imagen,'
